...
- The initial buses/routes/sends SOW retires a graph generation once all of its instruments finish because gain and filter have no independent tails. Reverb implementation must extend this lifecycle first: each bus effect needs a finite tail-duration contract (or another explicit completion mechanism), and generation retirement must wait for bus tails after the final source disconnects. Feedback or unbounded tails also require a documented maximum retirement policy. This is a prerequisite for adding reverb, not optional cleanup work.
